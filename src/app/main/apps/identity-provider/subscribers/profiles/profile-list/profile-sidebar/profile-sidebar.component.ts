...
export class ProfileSidebarComponent implements OnInit {
  ...
  addSubjectDNA() {
    ...
    this.getSelectSubjectDNA['checked'] = false;
  ...
  addSubjectAttribute() {
    ...
    this.newProfile.controls["subjectAttribute"].setValue(
      this.listSubjectAttribute
    );
  }
  ...
  removeSubjectATT() {
    this.indexCheckedATT.map((item) => {
      ...
      this.listSubjectAttribute = this.listSubjectAttribute.filter(
        (val, index) => {
          return index !== item;
        }
      );
