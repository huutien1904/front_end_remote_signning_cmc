...
@Component({
  selector: 'app-token-password-user',
  templateUrl: './token-password-user.component.html',
  styleUrls: ['./token-password-user.component.scss']
})

export class TokenPasswordUserComponent implements OnInit {
  ...
  onSubmit() {
    ...
    if (this.tokenForm.valid) {
      ...
      const newRequest = JSON.stringify({
        tokenInitPinOld: this.f.tokenOldPassword.value,
        tokenInitPinNew: this.f.tokenNewPassword.value,
      });
      ...
      Swal.fire({
        ...
        preConfirm: async () => {
          return await this._tokenService
            .updateTokenUserPassword(this.tokenDetail.tokenId, newRequest)
        ...
        if (result.value) {
          Swal.fire({
            icon: 'success',
            title: 'Thành công!',
            text: 'TOKEN đổi mật khẩu USER thành công.',
            customClass: {
